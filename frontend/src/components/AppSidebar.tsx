...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  return (
    <Sidebar
      collapsible="icon"
      variant="sidebar"
      className={`${state !== "collapsed" ? "w-[260px]" : ""
        } !bg-white !border-[#D4E162]`}
      {...props}
    >
      <SidebarHeader
        className={`!py-2 relative ${state !== "collapsed" ? "!px-5" : "!px-3"
          }`}
      >
        <div className="flex h-[50px] items-center gap-1 justify-start ">
          ...
          {state !== "collapsed" && (
            <div className="grid flex-1 text-left text-2xl leading-tight ml-px">
              <h2 className="truncate font-medium">Slotsy</h2>
            </div>
          )}

          <SidebarTrigger
            className={`-ml-1 cursor-pointer ${state === "collapsed" &&
